...
const login = _ => {
  ...
  ADMIN.socket.emit("admin-key", adminkeyInput.value, d => {
    if (d == "success") {
        ...
        ADMIN.socket.emit("db-get-users", users => {
            ...
            n.addEventListener("focus", e => {
              if (n.innerText == "username") {
                ...
                document.querySelector(
                  `#ubtn${e.target.dataset.index}`
                ).classList = "ubtn-dirty";
          ...
          document.querySelector("#ubtn999").addEventListener("click", e => {
            if (e.target.classList == "ubtn-dirty") {
              ...
              const pw = e.target.parentNode.querySelector(
                'span [class="editable nud pw"]'
              ).innerText;
          ...
          ADMIN.userBtns.forEach(btn => {
            btn.onclick = e => {
              if (btn.classList == "ubtn-dirty") {
                const name = btn.parentNode.querySelector("span").innerText;
                const pw = btn.parentNode.querySelector(
                  'span [class="editable"]'
                ).innerText;
      ...
      const currentPlayersCard = document.querySelector(
        "#current-players-card"
      );
      const currentPlayersCardBody = currentPlayersCard.querySelector(
        ".card-body"
      );
...
function isAlphaNumeric(e) {
  ...
  return (
    (k > 47 && k < 58) || (k > 64 && k < 91) || (k > 96 && k < 123) || k == 0
  );
}
